...
object CubeSolver {
    fun tryToSolveCube() : Cube?{
        val cube = Cube()
        val cursor = CubeCursor(cube)
        var index = 1
        while(!cube.isSolved()){
            val moves = cursor.getPossibleMoves()
            if(moves.isNotEmpty()){
                cursor.move(moves[moves.indices.random()])
                cube.setValue(cursor.getCurrentPosition(), index)
                index++
            }else{
                return null
            }
